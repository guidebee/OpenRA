...
namespace OpenRA.TemplateReader
{
	...
	{
		readonly Dictionary<string, List<MixFile>> tilesetMixFiles = new();
		readonly Dictionary<string, byte[]> extractedFileCache = new();
		...
		void LoadMixFiles()
		{
			...
			var supportDir = GetSupportDir();
			if (string.IsNullOrEmpty(supportDir))
				return;

			// Look for RA Content directory
			var raContentDir = Path.Combine(supportDir, "Content", "ra", "v2");
			...
			var cncContentDir = Path.Combine(supportDir, "Content", "cnc", "v2");
			...
			var ra2ContentDir = Path.Combine(supportDir, "Content", "ra2");
			...
			var raCncDir = Path.Combine(raContentDir, "cnc");
		...
		public byte[] GetTemplateFromMix(string tileset, string templateName)
		{
			...
			var cacheKey = $"{tileset}:{templateName}";
			if (extractedFileCache.TryGetValue(cacheKey, out var cachedData))
				return cachedData;
			...
			{
				var data = ExtractFile(tileset, name);
			...
			{
				var result = CheckAlternativeTilesets(tileset, templateName);
				if (result != null)
				{
		...
		/// </summary>
		List<string> GeneratePossibleTemplateNames(string tileset, string templateName)
		{
			var extension = GetTemplateExtension(tileset);
			var possibleNames = new List<string>
			{
				...
				$"{tileset}.{templateName}{extension}"
			};
		...
		/// </summary>
		static bool IsSpecialTemplate(string templateName)
		{
		...
		/// </summary>
		byte[] CheckAlternativeTilesets(string originalTileset, string templateName)
		{
			...
			foreach (var alternateTileset in new[] { "temperat", "snow", "winter", "interior", "desert" })
			{
				...
				foreach (var name in possibleNames)
				{
					var data = ExtractFile(alternateTileset, name);
					if (data != null && data.Length > 0)
					{
			...
			foreach (var generalTileset in new[] { "general", "local", "conquer" })
			{
				...
				foreach (var name in possibleNames)
				{
					var data = ExtractFile(generalTileset, name);
					if (data != null && data.Length > 0)
					{
		...
		static string GetTemplateExtension(string tileset)
		{
		...
		void LoadMixFile(string baseDir, string mixFileName, string tileset)
		{
			try
			{
				var mixFilePath = Path.Combine(baseDir, mixFileName);
			...
			foreach (var mixFile in mixFiles)
			{
				try
				{
					if (mixFile.Contains(filename))
					{
						...
						var data = mixFile.Extract(filename);
						extractedFileCache[cacheKey] = data; // Cache the result
						return data;
					}
				}
